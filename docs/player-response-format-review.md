...
此前发现的两处问题已修复：共享 `resourceUrl()` 剥离开头的 `relative/` 标记，玩家查询与档案名片图片统一使用正确地址；`last_logout_time` 在图片和文字中统一显示北京时间，兼容秒、毫秒及日期文本，零值/缺失/无效值显示“未知”。其他数字字段不进行日期转换。

`npm run test:player-response`：**14/14 通过**，包含两份给定示例、URL 兼容、时间边界、模板/文字一致性与旧 rows 回归。示例离线时间显示为 `2026-09-19 20:59:53（北京时间）`。未向生产服务器请求名片图片，因此资源在线可用性仍需部署环境确认。

下文保留修复前检查过程；当前结果见 [样本验证结果](player-response-review-results.json)。

修复前结论：主要嵌套结构已兼容，但名片图片地址有适配缺口，最后离线时间需要格式化。
