...
查蛋配种功能优先使用后端宠物图鉴和尺寸查询接口，并使用本地 `Pets.json` 兜底，不需要登录账号。

### `洛克.查蛋 <精灵名>`

查询指定精灵的蛋组、性别比、身高体重范围、种族值、同蛋组可配种精灵等信息。名称查询优先调用新版 `/wiki/pets`、`/wiki/pets/{pet_id}` 和 `/wiki/pets/{pet_id}/profile`，接口不可用时自动回退本地数据。
...
按尺寸反查精灵。身高使用游戏原生单位 `m`，体重使用 `kg`。同时提供身高和体重时，会调用新版 `/wiki/pet-size/query` 接口；接口不可用时自动回退到本地数据查询。
